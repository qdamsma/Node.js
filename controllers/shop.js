...
exports.getCheckout = (req, res, next) => {
    ...
    req.user.populate('cart.items.productId')
        ...
        .then(session => {
            res.render('shop/checkout', { pageTitle: 'Checkout', path: '/checkout', products: products, totalSum: total, stripePublishableKey: process.env.STRIPE_PUBLISHABLE_KEY, sessionId: session.id });
        })
        .catch(err => {
            const error = new Error(err);
            error.httpStatusCode = 500;
